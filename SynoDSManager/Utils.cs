...
namespace SynoDSManager
{
    public class Utils
    {
        ...
        public static void SendAlertEmail( EmailSettings settings, IEnumerable<SynoTask> tasks )
        {
            string body = "Cleaned up the following downloads:\n";
            foreach (var task in tasks.OrderBy(x => x.title))
            {
                var msg = $" - {task.title}: {task.status} (Tracker: {task.additional.tracker.FirstOrDefault().url})";
                body += msg + "\n";
            }
